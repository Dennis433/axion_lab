...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Syne:wght@700;800&family=JetBrains+Mono:wght@400;500;600;700&family=Manrope:wght@400;500;600;700&display=swap" rel="stylesheet">
...
:root {
  ...
  --hairline: #33215c;
  --signal: #39ff88;
  --signal-dim: rgba(57, 255, 136, 0.14);
  --pulse: #ff4d6d;
  --pulse-dim: rgba(255, 77, 109, 0.14);
  --brand: #c13cff;
  --brand-dim: rgba(193, 60, 255, 0.16);
  --paper: #f3eeff;
  --dim: #9c8cc4;
  --radius-sm: 4px;
  --radius-md: 8px;
...
.flash-success { background: var(--brand-dim); color: var(--brand); border-color: rgba(193,60,255,.35); }
.flash-error { background: var(--pulse-dim); color: var(--pulse); border-color: rgba(255,77,109,.35); }
...
.coin-table tbody tr:hover { background: rgba(193,60,255,0.06); }
...
const MemeDex = (() => {
  ...
  function tokenIconUrl(t) {
    ...
    return `https://api.dicebear.com/7.x/identicon/svg?seed=${seed}&backgroundColor=160b2e`;
  }
...
def get_trending(chain_id: str = None, limit: int = 80):
    ...
    # Primary, reliable source: many diversified search terms merged
    # together. A single term (e.g. "meme") biases every result toward that
    # literal word — a wide spread of terms gives real breadth and volume.
    search_terms = (
        "pepe", "doge", "wif", "bonk", "trump", "cat", "inu", "moon",
        "shiba", "floki", "elon", "frog", "baby", "safe", "coin", "wojak",
        "chad", "based", "turbo", "ai", "meme", "dog", "pump", "rocket",
    )
...
for f in app/templates/base.html app/static/css/style.css app/static/js/app.js app/dex_api.py; do
