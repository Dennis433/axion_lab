...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Unbounded:wght@600;700;800&family=JetBrains+Mono:wght@400;500;600;700&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet">
...
<body>
  ...
  <footer class="footer">
    ...
    <div class="footer-tag">Built with Flask · Bootstrap · DEX data via DexScreener · Swaps via 0x</div>
  </footer>
...
:root {
  ...
  --hairline: #2b2820;
  --signal: #f2a93b;
  --signal-dim: rgba(242, 169, 59, 0.14);
  --pulse: #e14f4f;
  --pulse-dim: rgba(225, 79, 79, 0.14);
  --paper: #ede7da;
  --dim: #8f8878;
  --radius-sm: 4px;
  --radius-md: 8px;
...
.btn-outline { background: transparent; color: var(--signal); }
.btn-outline:hover { background: var(--signal-dim); }
.btn-solid { background: var(--signal); color: #1a1509; border: 1px solid var(--signal); }
...
.flash-success { background: var(--signal-dim); color: var(--signal); border-color: rgba(242,169,59,.3); }
.flash-error { background: var(--pulse-dim); color: var(--pulse); border-color: rgba(225,79,79,.3); }
...
.chip-active { color: var(--signal); border-color: var(--signal); background: var(--signal-dim); }
...
.coin-table tbody tr:hover { background: rgba(242,169,60,0.04); }
...
.copy-confirm { opacity: 0; color: var(--signal); font-size: 12.5px; margin-top: 8px; transition: opacity .2s; font-family: var(--font-mono); }
...
.chain-explorer { font-size: 11.5px; color: var(--signal); font-family: var(--font-mono); }
...
.trade-tab-active { background: var(--signal-dim); color: var(--signal); border-color: var(--signal); }
...
.chat-fab {
  width: 50px; height: 50px; border-radius: var(--radius-sm); background: var(--signal);
...
.chat-option {
  ...
  border: 1px solid var(--signal); color: var(--signal); border-radius: var(--radius-sm);
  padding: 8px 12px; font-size: 12.5px; margin-bottom: 6px; cursor: pointer;
  font-family: var(--font-mono);
}
.chat-option:hover { background: var(--signal-dim); }
...
const MemeDex = (() => {
  ...
  function tokenIconUrl(t) {
    ...
    return `https://api.dicebear.com/7.x/identicon/svg?seed=${seed}&backgroundColor=15130e`;
  }
  ...
  async function fetchTokens({ q = "", chain = "" } = {}) {
    ...
    try {
      ...
      if (!res.ok) {
        console.error(`MemeDex: /api/tokens returned ${res.status}`, await res.text());
        if (tbody) tbody.innerHTML = `<tr><td colspan="4" class="empty-row">Server error loading tokens (status ${res.status}). Check console/Render logs.</td></tr>`;
        return;
      }
      ...
      if (err.name !== "AbortError") {
        ...
        if (tbody) tbody.innerHTML = `<tr><td colspan="4" class="empty-row">Couldn't load tokens right now (see console for details).</td></tr>`;
      }
    }
  }
...
def get_trending(chain_id: str = None, limit: int = 30):
    """
    Discovery list for the homepage/ticker.

    Primary source is a set of diversified search terms against DexScreener's
    search endpoint (proven reliable — this is the same call `search_tokens`
    uses). We also opportunistically try the token-boosts endpoint for a
    bonus signal, but never depend on it alone: if DexScreener changes that
    endpoint's shape or it's empty, the page still shows real tokens instead
    of nothing.
    """
    results = []
    ...
    # Primary, reliable source: diversified search terms merged together.
    # A single term (e.g. "meme") biases every result toward that literal
    # word — several varied terms give a much more representative list.
    for term in ("pepe", "doge", "wif", "bonk", "trump", "cat", "inu", "moon"):
        results.extend(search_tokens(term, limit=15))
    ...
    if not deduped:
        log.error("get_trending produced zero results (chain_id=%r)", chain_id)
...
for f in app/templates/base.html app/static/css/style.css app/static/js/app.js app/dex_api.py app/routes/api.py; do
