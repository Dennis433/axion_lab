#!/usr/bin/env bash
set -e
echo "Applying fix + new color theme..."
...
# Simple in-memory caches. The homepage table polls every 20s and the ticker
# polls every 30s, and each trending refresh fans out to ~24 search requests
# against DexScreener — without caching, that volume was crowding out the
# on-demand "click a token" detail requests and causing intermittent
# "couldn't load this token" failures from rate limiting. Caching each
# distinct query for a short window cuts the real request volume drastically
# while still feeling live to the user.
_trending_cache = {}  # key: chain_id or "*"  -> (timestamp, data)
_TRENDING_TTL = 45  # seconds
...
def get_trending(chain_id: str = None, limit: int = 80):
    ...
    per chain filter so repeated polling doesn't re-fan-out to DexScreener
    on every request.
    """
...
def get_pair(chain_id: str, pair_address: str):
    ...
    try:
        ...
        return None  # don't cache failures — let an immediate retry succeed
...
for f in app/static/css/style.css app/static/js/app.js app/dex_api.py; do
